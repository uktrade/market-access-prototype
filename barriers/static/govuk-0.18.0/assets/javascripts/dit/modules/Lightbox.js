define(['modules/overlay'],function(overlay) {
    ...
    var template = _.template('<% if(title) { %><div class="title"><h1><%= title %></h1></div><% } %><div class="outer"><span><%= (message && (typeof(message) == "function" ? message(obj.data) : message)) || "" %></span></div>\
        ...
        <% if(buttons.yes){ %><button type="button" value="yes" class="button btn-primary dlg-close"><%=buttons.yes.txt || "Yes" %></button><% } %>\
        <% if(buttons.no){ %><button type="button" value="no" class="button btn-default dlg-close"><%=buttons.no.txt || "No" %></button><% } %>\
        </div>');
